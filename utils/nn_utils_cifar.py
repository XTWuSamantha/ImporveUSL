...
def train_dataset_medmnist(transform_name, load_local_global_dataset=False):
    if transform_name =="medMNIST":
        normalization_kwargs = normalization_kwargs_dict[transform_name]
        transform_train = transforms.Compose([
            transforms.RandomHorizontalFlip(),
    ...
    if not load_local_global_dataset:
        if cfg.DATASET.NAME == "bloodmnist":
            ...
            train_dataset_cifar = DataClass(split='train', transform=transform_train,  download=True, index = None)
            val_dataset = DataClass(split='test', transform=transform_val,  download=True, index = None)

            sample_db = make_imb_data(243, 8, 1)
            imb_idxs = createImbIdxs(val_dataset.labels, sample_db)
            random.seed(10)
            random.shuffle(imb_idxs)

            val_dataset = DataClass(split='test', transform=transform_val,  download=False, index = imb_idxs)
        ...
        if cfg.DATASET.NAME == "bloodmnist":
            ...
            train_dataset_cifar = DataClass(split='train', transform=None,  download=True, index = None)
            val_dataset = DataClass(split='test', transform=transform_val,  download=True, index = None)
